...
export const createOrder = async (req, res) => {
  try {
    ...
    if (req.body.addressId && cart.selectedAddress?.toString() !== req.body.addressId) {
      const newAddress = await Address.findOne({ _id: req.body.addressId, user: userId, deletedAt: null });
      if (!newAddress) {
        return res.status(400).json(responseHandler.error("Address from request body is invalid"));
      }
...
export const getOrderById = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(orderId)) {
      return res.status(400).json(responseHandler.error("Invalid Order ID"));
    }
  ...
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        ...
        if (start > today) return res.status(400).json(responseHandler.error("Future dates are not allowed"));
        query.createdAt.$gte = start;
      }
      if (endDate) {
        const end = new Date(endDate);
        end.setHours(23, 59, 59, 999);
        if (end > today) return res.status(400).json(responseHandler.error("Future dates are not allowed"));
        query.createdAt.$lte = end;
      }

      if (query.createdAt.$gte && query.createdAt.$lte) {
        if (query.createdAt.$lte < query.createdAt.$gte) {
